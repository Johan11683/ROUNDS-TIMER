...
export const CircularTimer: React.FC<CircularTimerProps> = ({ remainingSeconds, totalSeconds, isRunning, size = 360 }) => {
  ...
  const progress = useMemo(() => {
    const ratio = totalSeconds > 0 ? Math.max(0, Math.min(1, 1 - remainingSeconds / totalSeconds)) : 0;
    return ratio;
  ...
  const ticks = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      arr.push(<line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="rgba(255,255,255,0.15)" strokeWidth={i % 5 === 0 ? 2 : 1} />);
    }
    return arr;
  }, [r, size]);
